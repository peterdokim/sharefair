...
async function saveDatabaseSettlementRequest(requestInput) {
  ...
  await sql`
    INSERT INTO settlement_requests (
      ...
      status, created_at, settled_at, initial_sent_at, reminder_3h_sent_at, reminder_15m_sent_at
    )
    VALUES (
      ...
      ${request.createdAt}, ${request.settledAt}, ${request.initialSentAt}, ${request.reminder3hSentAt}, ${request.reminder15mSentAt}
    )
...
export async function markSettlementRequestSettled(tripId, requestId) {
  const requests = await listSettlementRequestsForTrip(tripId);
...
export async function processSettlementReminderQueue(nowInput = new Date()) {
  ...
  for (const request of requests) {
    ...
    if (currentStatus === "overdue" && request.status !== "overdue") {
      nextRequest = normalizeRequest({ ...nextRequest, status: "overdue" });
      overdueMarked += 1;
    }

    let reminderType = null;
    if (nextRequest.status !== "settled") {
