...
function getParticipantIdentity(participant) {
  ...
  return `name:${participant.name.toLowerCase()}|email:${participant.email}`;
}
...
function validateTripInput(input, options = {}) {
  const existingParticipants = Array.isArray(options.existingParticipants) ? options.existingParticipants : [];
  ...
  const participants = (input?.participants || [])
    ...
    .filter((participant) => participant.id || participant.name || participant.email);
  ...
  for (const participant of participants) {
    if (!participant.name) {
      throw createError(400, "Each traveler needs a name before you save the trip.");
    }
  }

  const dedupedParticipants = participants.filter(
    (participant, index, collection) =>
      collection.findIndex((entry) => getParticipantIdentity(entry) === getParticipantIdentity(participant)) === index
  );
  ...
  for (const existingId of existingParticipantIds) {
    if (!dedupedParticipants.some((participant) => participant.id === existingId)) {
      throw createError(400, "Existing travelers cannot be removed here yet. Fix their name or email instead.");
    }
  }
...
async function updateTripInDatabase(tripId, input) {
  await ensureDatabaseSchema();
  const sql = getSql();
  ...
  const validated = validateTripInput(input, {
    existingParticipants: existingTrip.participants
  });
  const existingParticipantIds = new Set(existingTrip.participants.map((participant) => participant.id));
  ...
  for (const participant of validated.participants) {
    const participantId = existingParticipantIds.has(participant.id) ? participant.id : createId("person");
    ...
    if (existingParticipantIds.has(participantId)) {
      ...
      await sql`
        INSERT INTO participants (id, trip_id, name, email, created_at)
        VALUES (${participantId}, ${tripId}, ${participant.name}, ${participant.email || null}, ${new Date().toISOString()})
      `;
    }
  }
...
export async function updateTrip(tripId, input) {
  if (isDatabaseConfigured()) {
    return updateTripInDatabase(tripId, input);
  }
  ...
  await updateLocalState((state) => {
    const trips = state.trips.map((trip) => {
      ...
      const validated = validateTripInput(input, {
        existingParticipants: normalizedTrip.participants
      });
      const existingParticipantIds = new Set(normalizedTrip.participants.map((participant) => participant.id));
      const nextParticipants = validated.participants.map((participant) => ({
        id: existingParticipantIds.has(participant.id) ? participant.id : createId("person"),
        name: participant.name,
        email: participant.email || null
      }));
