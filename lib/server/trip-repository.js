...
function normalizePayment(payment) {
  return {
    ...
    debugStepUpCode: payment.debugStepUpCode || null
  };
}
...
function validateTripInput(input) {
  ...
  const participants = [...new Set((input?.participants || []).map((participant) => String(participant || "").trim()).filter(Boolean))];
...
async function createTripInDatabase(input) {
  ...
  const participants = validated.participants.map((name) => ({
    id: createId("person"),
    name
  }));
  ...
  for (const participant of participants) {
    await sql`
      ...
      VALUES (${participant.id}, ${tripId}, ${participant.name}, ${createdAt})
    `;
  }
...
export async function createTrip(input) {
  ...
  const trip = normalizeTrip({
    ...
    participants: validated.participants.map((name) => ({
      id: createId("person"),
      name
    })),
    expenses: [],
