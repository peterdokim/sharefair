...
function buildMockEmailAddress(actor) {
  const fallbackLocalPart = actor?.participantId || "traveler";
  const safeLocalPart =
    String(actor?.participantName || fallbackLocalPart)
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, ".")
      .replace(/^\.+|\.+$/g, "") || fallbackLocalPart;

  return `${safeLocalPart}@sharefair-mail.test`;
}
...
function createStepUpChallenge(actor) {
  ...
  const destinationEmail = buildMockEmailAddress(actor);
  ...
  return {
    ...
    status: "required",
    verifiedAt: null,
    debugCodePreview: process.env.NODE_ENV === "production" ? null : code
  };
}
...
export async function createPaymentAuthorizationSession(input) {
  ...
  if (!validParticipantIds.has(fromParticipantId) || !validParticipantIds.has(toParticipantId)) {
  ...
  const session = {
    ...
    expiresAt,
    stepUp: createStepUpChallenge(actor),
  ...
  const payment = await savePaymentRecord(tripId, {
    ...
    verifiedAt: null,
    stepUpRequired: true,
    stepUpChallengeId: session.stepUp.id,
    stepUpMaskedEmail: session.stepUp.maskedDestination,
    stepUpExpiresAt: session.stepUp.expiresAt,
    ...
    debugStepUpCode: session.stepUp.debugCodePreview
  });
...
export async function resendStepUpChallenge(input) {
  ...
  await persistPaymentPatch(updatedSession, {
    ...
    authorizationStatus: updatedSession.authorizationStatus,
    stepUpRequired: true,
    stepUpChallengeId: refreshedChallenge.id,
    stepUpMaskedEmail: refreshedChallenge.maskedDestination,
    stepUpExpiresAt: refreshedChallenge.expiresAt,
    stepUpAttemptsRemaining: refreshedChallenge.attemptsRemaining,
    stepUpStatus: refreshedChallenge.status,
    stepUpVerifiedAt: null,
    debugStepUpCode: refreshedChallenge.debugCodePreview
  });
